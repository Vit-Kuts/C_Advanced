...
void FLAGS_FlagAnalysis(int arg, char *argv[]) {
  ...
  while ((rez = getopt_long(arg, argv, "hirat", long_options, &option_index)) !=
         -1) {
    switch (rez) {
      case 'h':
        temp.help = 1;
...
static void print_help(void) {
  ...
  printf(
      "  -intersections     - find and print intersection points of curves\n");
  printf(
      "  -iterations        - print number of iterations for root finding\n");
  printf("  -area              - calculate area bounded by curves\n");
  printf("  -test              - run tests for root and integral functions\n");
  printf("\n");
  printf("Combined usage:\n");
  printf(
      "  program -intersections -iterations  # find intersections with "
      "iteration count\n");
}
