...
#define CONTROLS 2  // Количество наборов управления
...
// Структура для хранения кодов управления
...
struct control_buttons default_controls[CONTROLS] = {
    {KEY_DOWN, KEY_UP, KEY_LEFT, KEY_RIGHT}, {'s', 'w', 'a', 'd'}};
...
int checkDirection(snake_t *snake, int32_t key) {
  ...
  // Определяем запрошенное направление
...
int main() {
  ...
  mvprintw(INFO_Y, 1, "Use arrows or WASD for control. Press 'F10' for EXIT");
  ...
  while (key_pressed != STOP_GAME) {
    ...
    // Поддержка верхнего регистра для WASD
    if (key_pressed >= 'A' && key_pressed <= 'Z') {
      key_pressed = key_pressed - 'A' + 'a';
    }
