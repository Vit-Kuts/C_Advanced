...
static double find_intersection(func_t f, func_t g, deriv_t df, deriv_t dg, double a,
                         double b, double eps, int *iters) {
  return ROOT_root(f, g, a, b, df, dg, eps, iters);
}
...
void CALCULATE_compute_area(void) {
  ...
  /* Поиск всех точек пересечения */
  x_f1f2 = ROOT_root(f1, f2, 0.0, 10.0, df1, df2, 1e-7, &iters);
  x_f1f3 = ROOT_root(f1, f3, 0.5, 5.0, df1, df3, 1e-7, &iters);
  x_f2f3 = ROOT_root(f2, f3, 3.0, 4.0, df2, df3, 1e-7, &iters);
  ...
  /* Разностные функции для интегрирования */
  double diff_f1_f3(double x) { return f1(x) - f3(x); }
  double diff_f2_f3(double x) { return f2(x) - f3(x); }

  /* Вычисление площади */
  double area_part1 = INTEGRAL_integral(diff_f1_f3, left, mid, eps);
  double area_part2 = INTEGRAL_integral(diff_f2_f3, mid, right, eps);
