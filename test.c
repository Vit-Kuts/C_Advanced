...
void run_tests(void) {
    ...
    double test_g(double x) { (void)x; return 4.0; }
    double test_df(double x) { return 2.0 * x; }
    double test_dg(double x) { (void)x; return 0.0; }
    
    result = root(test_f, test_g, 1.0, 3.0, test_df, test_dg, eps, &iters);
    ...
    result = integral(square, 0.0, 2.0, eps);
    printf("  Result: %.10f\n", result);
    ...
    printf("\nTest 3: Integrate sin(x) from 0 to π (expected 2.0)\n");
    double sine(double x) { return sin(x); }
    
    result = integral(sine, 0.0, M_PI, eps);
    printf("  Result: %.10f\n", result);
